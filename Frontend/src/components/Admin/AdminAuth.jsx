...
export default function AuthorityTable() {
  ...
  const [authorities, setAuthorities] = useState([
    { name: "John Doe", email: "john@example.com", department: "Water" },
    { name: "Jane Smith", email: "jane@example.com", department: "Electricity" },
    { name: "Mike Brown", email: "mike@example.com", department: "Municipal Counsils" },
    { name: "Lisa White", email: "lisa@example.com", department: "Water" },
  ]);
  ...
  const handleDelete = (email) => {
    setAuthorities(authorities.filter(auth => auth.email !== email));
  };
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {filtered.map((auth, idx) => (
                  <tr key={auth.email} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    <td className="px-4 py-2">{auth.name}</td>
                    <td className="px-4 py-2">{auth.email}</td>
                    <td className="px-4 py-2">{auth.department}</td>
